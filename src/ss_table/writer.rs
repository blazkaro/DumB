...
impl SsTableWriter {
    pub async fn init(
        ...
        dma_writer.write_all(&level.to_le_bytes()).await?;
        Ok(Self {
            dma_writer,
            storage_config,
        })
    ...
    pub async fn write_entry(
        ...
        self.dma_writer
            ...
            .await?; // Key length
        self.dma_writer.write_all(&key).await?; // Key

        match &value {
            DbValue::Value(bytes) => {
                self.dma_writer.write_all(&[0u8]).await?; // Value type
                self.dma_writer
                    .write_all(&(bytes.len() as u32).to_le_bytes())
                    .await?; // Value length in bytes
                self.dma_writer.write_all(bytes).await?; // Value
            }
            DbValue::Tombstone => {
                self.dma_writer.write_all(&[1u8]).await?;
            }
        }
    ...
    pub async fn finish(mut self) -> Result<(), GlommioError<()>> {
